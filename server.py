...
async def shutdown_event():
    """Clean up the global httpx client pool on shutdown."""
...
# Global httpx client pool — reused for keep-alive, eliminates per-request SSL handshake
...
GEMINI_THOUGHT_SIGNATURE = "skip_thought_signature_validator"
...
def contains_thinking_marker(text: Any) -> bool:
    """Check if text/data contains Gemini/Anthropic thinking or signature markers."""
    if text is None:
        return False
    text = str(text)
    markers = [
...
def inject_thought_signatures(messages: list) -> list:
    """Inject skip_thought_signature_validator into assistant tool_calls.

    Gemini 3.x requires thought_signature on every assistant tool_call in
    multi-turn conversations. Without it, the API rejects the request.
    Same approach as OpenclawVertexProxyService.ensureThoughtSignatures().
    """
    patched = 0
    for msg in messages:
        role = msg.get("role", "")
        ...
        for tc in tool_calls:
            ...
            if not google.get("thought_signature"):
                google["thought_signature"] = GEMINI_THOUGHT_SIGNATURE
                patched += 1
    if patched:
        logger.info(f"Injected thought_signature into {patched} tool_calls")
    return messages

async def handle_gemini_openai_streaming(
    openai_body: dict,
    original_request,
    api_key: str,
    original_model_name: str = "",
) -> AsyncGenerator[str, None]:
    """Stream Gemini OpenAI-compatible endpoint, convert to Anthropic SSE.

    Bypasses LiteLLM entirely. Direct HTTP/SSE, just like Openclaw Java service.
    Filters empty-content thinking chunks and strips extra_content.
    """
    ...
    # Use original_model_name (e.g. claude-opus-4-8) not the mapped model
    # so cc-switch's RECT-001 rectifier doesn't see a non-Anthropic model name
    display_model = original_model_name or getattr(original_request, 'model', 'unknown')
    message_id = f"msg_{uuid.uuid4().hex[:24]}"
    yield f"event: message_start\ndata: {json.dumps({'type': 'message_start', 'message': {'id': message_id, 'type': 'message', 'role': 'assistant', 'model': display_model, 'content': [], 'stop_reason': None, 'stop_sequence': None, 'usage': {'input_tokens': 0, 'cache_creation_input_tokens': 0, 'cache_read_input_tokens': 0, 'output_tokens': 0}}})}\n\n"
    yield f"event: content_block_start\ndata: {json.dumps({'type': 'content_block_start', 'index': 0, 'content_block': {'type': 'text', 'text': ''}})}\n\n"
    yield f"event: ping\ndata: {json.dumps({'type': 'ping'})}\n\n"

    tool_index = -1  # -1 means no tool started yet
    current_tool_index = -1  # the OpenAI tool_call index we're tracking
    text_sent = False
    ...
    async with client.stream("POST", url, headers=headers, json=openai_body) as resp:
        if resp.status_code != 200:
            body = await resp.aread()
            logger.error(f"Gemini OpenAI endpoint returned {resp.status_code}: {body[:500]}")
        ...
        async for line in resp.aiter_lines():
            ...
            # ---- detect thinking wrapper ----
            delta_content = delta.get("content")
            delta_tool_calls = delta.get("tool_calls")
            has_role = bool(delta.get("role"))

            is_thinking_wrapper = (
                (delta_content == "" or delta_content is None)
                and not delta_tool_calls
                and not has_role
            )
            if is_thinking_wrapper:
                continue  # skip pure thinking chunk

            # ---- text content ----
            if delta_content is not None and delta_content != "":
                accumulated_text += delta_content
                if tool_index == -1 and not text_block_closed:
                    text_sent = True
            ...
            if delta_tool_calls:
                ...
                for tc in delta_tool_calls:
                    ...
                    if tc_index != current_tool_index:
                        ...
                        tool_index = tc_index
                        anthropic_tool_index = tc_index + 1
                        tc_id = tc.get("id", f"toolu_{uuid.uuid4().hex[:24]}")
                        yield f"event: content_block_start\ndata: {json.dumps({'type': 'content_block_start', 'index': anthropic_tool_index, 'content_block': {'type': 'tool_use', 'id': tc_id, 'name': tc_name, 'input': {}}})}\n\n"

                    if tc_args:
                        yield f"event: content_block_delta\ndata: {json.dumps({'type': 'content_block_delta', 'index': anthropic_tool_index, 'delta': {'type': 'input_json_delta', 'partial_json': tc_args}})}\n\n"
            ...
            if finish_reason and not has_sent_stop:
                ...
                # Close tool blocks
                if tool_index >= 0:
                    for i in range(1, anthropic_tool_index + 1):
                ...
                stop_reason = "tool_use" if tool_index >= 0 else "end_turn"
    ...
    # fallback close if no finish_reason seen
...
class MessagesRequest(BaseModel):
    ...
    def validate_model_field(cls, v, info): # Renamed to avoid conflict
        ...
        # Map Fable to BIG_MODEL based on provider preference
        elif 'fable' in clean_v.lower():
            if PREFERRED_PROVIDER == "google" and BIG_MODEL in GEMINI_MODELS:
                new_model = f"gemini/{BIG_MODEL}"
                mapped = True
            else:
...
def convert_to_gemini_openai(anthropic_request: MessagesRequest) -> dict:
    """Convert Anthropic Messages request to OpenAI Chat Completions format
    for Gemini OpenAI-compatible endpoint. Mirrors OpenclawJava service logic."""
    ...
    if anthropic_request.system:
        ...
        if isinstance(anthropic_request.system, str):
            sys_text = anthropic_request.system
    ...
    for m in anthropic_request.messages:
        r = m.role if m.role == "assistant" else "user"
        content = m.content
        if isinstance(content, str):
            ...
            for blk in content:
                ...
                if btype == "text":
                    t = blk.get("text", "") if isinstance(blk, dict) else getattr(blk, "text", "")
                    if r == "assistant":
                        assistant_text_parts.append(t)
                    else:
                        user_text_parts.append(t)
                elif btype == "tool_use":
                    tc_id = blk.get("id", "") if isinstance(blk, dict) else getattr(blk, "id", "")
                    tc_name = blk.get("name", "") if isinstance(blk, dict) else getattr(blk, "name", "")
                    tc_input = blk.get("input", {}) if isinstance(blk, dict) else getattr(blk, "input", {})
                    tool_calls.append({
                        "id": tc_id,
                        "type": "function",
                        "function": {
                    ...
                    tid = blk.get("tool_use_id", "") if isinstance(blk, dict) else getattr(blk, "tool_use_id", "")
                    tc = blk.get("content", "") if isinstance(blk, dict) else getattr(blk, "content", "")
                    tc_text = ""
                    if isinstance(tc, list):
                        for item in tc:
                            if isinstance(item, dict) and item.get("type") == "text":
                                tc_text += item.get("text", "") + "\n"
            ...
            if r == "assistant":
                ...
                # If there are user text parts, emit a user message first
                if user_text_parts or not tool_results:
                    msgs.append({
                        "role": "user",
                        "content": "\n".join(user_text_parts) if user_text_parts else "..."
                    })
                # Then append all tool results as individual tool messages
    ...
    body = {
        ...
        "messages": inject_thought_signatures(msgs),
...
def convert_anthropic_to_litellm(anthropic_request: MessagesRequest) -> Dict[str, Any]:
    ...
    # Remove thinking to avoid Gemini thought_signature responses
    # cc-switch rectifier hangs on those signatures in stream chunks
...
def convert_litellm_to_anthropic(litellm_response: Union[Dict[str, Any], Any], 
    ...
    try:
        ...
        # Drop text that contains thought/signature markers to avoid cc-switch RECT-001 hangs
        if content_text is not None and content_text != "":
            if not contains_thinking_marker(content_text):
        ...
        if tool_calls and is_claude_model:
            ...
            for idx, tool_call in enumerate(tool_calls):
                ...
                # Drop thought/signature tool calls and argument payloads
                if (
                    '__thought__' in str(tool_id)
                    or name == '__thought__'
                    or contains_thinking_marker(tool_id)
                    or contains_thinking_marker(name)
                    or contains_thinking_marker(arguments)
                ):
                    logger.debug(f"Dropping thought tool call: id={tool_id}, name={name}")
                    continue
            ...
            for idx, tool_call in enumerate(tool_calls):
                ...
                if (
                    ...
                    logger.debug(f"Dropping thought tool call in non-Claude branch: id={tool_id}, name={name}")
                    continue
        ...
        async for chunk in response_generator:
            try:
                ...
                if hasattr(chunk, 'choices') and len(chunk.choices) > 0:
                    ...
                    # Strip provider_specific_fields / extra_content from delta
                    # Gemini 3.5 always injects thought_signature here, which
                    # cc-switch RECT-001 detects and kills the stream
                    ...
                    # Skip empty-content chunks that are Gemini thinking wrappers
                    # (no role, no tool_calls, content is "" or None)
                    if (delta_content == "" or delta_content is None):
                        has_tc = False
                        if hasattr(delta, 'tool_calls') and delta.tool_calls:
                            has_tc = True
                        elif isinstance(delta, dict) and delta.get('tool_calls'):
                            has_tc = True
                        has_role = False
                        if hasattr(delta, 'role') and delta.role:
                            has_role = True
                        elif isinstance(delta, dict) and delta.get('role'):
                            has_role = True
                        if not has_tc and not has_role:
                            continue  # pure thinking wrapper, skip

                    # Accumulate text content
                    if delta_content is not None and delta_content != "":
                        # Drop text deltas that contain thought/signature markers
                        if contains_thinking_marker(delta_content):
                            continue
                    ...
                    if delta_tool_calls:
                        ...
                        for tool_call in delta_tool_calls:
                            # Get the tool call ID/name to detect thought blocks
                            if isinstance(tool_call, dict):
                                tc_id = tool_call.get('id', '')
                                function = tool_call.get('function', {})
                                tc_name = function.get('name', '') if isinstance(function, dict) else ''
                            else:
                                tc_id = getattr(tool_call, 'id', '')
                                function = getattr(tool_call, 'function', None)
                                tc_name = getattr(function, 'name', '') if function else ''

                            # Skip Gemini thought blocks (LiteLLM wraps thoughtSignatures as __thought__ tool calls)
                            # Also drop any tool call whose id/name/arguments leak thought/signature data
                            if (
                                '__thought__' in str(tc_id)
                                or tc_name == '__thought__'
                                or contains_thinking_marker(tc_id)
                                or contains_thinking_marker(tc_name)
                            ):
                                continue
                            current_index = None
                            ...
                            if arguments:
                                # Drop argument fragments that contain thought/signature markers
                                if contains_thinking_marker(arguments):
                                    continue
...
async def create_message(
    ...
    try:
        ...
        # Strip thinking params to prevent Gemini thought_signature in responses
        # cc-switch's RECT-001 rectifier hangs on signatures in stream chunks
        litellm_request.pop("thinking", None)

        # For Gemini models: set reasoning_effort=disable so LiteLLM sends
        # thinkingBudget=0 + includeThoughts=False, preventing thoughtSignature
        # in the response that cc-switch RECT-001 chokes on
        if litellm_request["model"].startswith("gemini/"):
            litellm_request["reasoning_effort"] = "disable"
        ...
        if request.stream:
            ...
            # --- Gemini OpenAI direct path (bypass LiteLLM) ---
            if IS_GEMINI_OPENAI and target_model.startswith("openai/"):
                openai_body = convert_to_gemini_openai(request)
                logger.debug(f"Gemini OpenAI direct: {len(openai_body.get('messages',[]))} msgs, {len(openai_body.get('tools', []))} tools")
                return StreamingResponse(
                    handle_gemini_openai_streaming(openai_body, request, OPENAI_API_KEY, original_model),
